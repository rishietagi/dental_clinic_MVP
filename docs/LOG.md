...
**Where we are:** **PHASE 1 IS COMPLETE** (1.1–1.4 done). 1.1 = Supabase Auth + login page.
1.2 = `staff_user` model (`roles` array) + migration + admin seed. 1.3 = backend verifies the
Supabase JWT (ES256/JWKS) + role guards + role-aware nav. 1.4 = `audit_log` table +
`record_audit` service + the seed writes an audit entry. **Next is Phase 2 — Patients**,
starting with **step 2.1** (`patient` model + migration). Three migrations now
(empty root → staff_user → audit_log). `record_audit` is the hook every Phase 2 mutation
should call.
